...
def split_video(input_video, num_chunks, output_dir):
    LOGGER.info(f"Starting video splitting for '{input_video}' with {num_chunks} chunks...")
    try:
    ...
    try:
        ...
        LOGGER.info("Video successfully split into chunks.")
    except (subprocess.CalledProcessError, Exception) as e:
        raise Exception(f"An unexpected error occurred: {e}")
...
def create_hdfs_directory(base_dir, sub_dirs, workers):
    for sub_dir in sub_dirs:
        ...
        try:
            ...
            LOGGER.info(f"Created HDFS directory: {full_path}")
            
            for user in workers:
                acl_result = subprocess.run(['/opt/hadoop/bin/hadoop', 'fs', '-setfacl', '-R', '-m', f"user:{user}:rwx", full_path],
                                            check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                LOGGER.info(f"Set ACLs for {user} on {full_path}")
...
def upload_chunks_to_hdfs(chunk_files, hdfs_chunks_dir, num_chunks):
    ...
    def upload_chunk(chunk_path):
        ...
        LOGGER.info(f"Uploading chunk '{chunk_filename}' from path: {chunk_path_clean}")
        try:
            result = subprocess.run(['/opt/hadoop/bin/hadoop', 'fs', '-put', '-f', chunk_path_clean, hdfs_chunks_dir],
                                    check=True, stdout=subprocess.PIPE,
                stderr=subprocess.PIPE, text=True)
            LOGGER.info(f"Uploaded '{chunk_filename}' to HDFS.")
            LOGGER.info(f"HDFS put Output for '{chunk_filename}': {result.stdout}")
...
def process_video_file(index_and_info, input_video, hdfs_frame_dir):
    ...
    def extract_and_upload_frames(tmpdir, local_video_path, frame_offset, archive_filename):
        try:
            ...
            LOGGER.info(f"Extracting frames from {local_video_path} to {frame_output_pattern}")
            result = subprocess.run(
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                LOGGER.info(f"Retrieved {hdfs_path} to {local_video_path}")
            except Exception as e:
            ...
            LOGGER.info(f"Finished processing video: {hdfs_path}")
...
def main():
    input_video = "10-min.mp4"
    num_chunks = 10
    workers = ['user-2', 'user-3']
    
    conf = SparkConf() \
    .setAppName(f"Video Processing | {input_video} | Chunks: {num_chunks}") \
    ...
    try:
        ...
        LOGGER.info("Video splitting and uploading completed successfully.")
        ...
        LOGGER.info("Frame extraction and uploading completed successfully.")
