...
class VideoLogoReplacer:
    def __init__(self, input_video, old_logo, new_logo, output_dir="output"):
        ...
        self.frames_dir = self.output_dir / 'frames_full-v3'
        self.processed_dir = self.output_dir / 'processed_frames_full-v3'
        self.checkpoint_dir = self.output_dir / 'checkpoints_full-v3'
        self.metrics_file = self.output_dir / 'full-v3_metrics.json'
    ...
    def extract_frames(self):
        """Extract frames using ffmpeg with multi-threading."""
        start_time = time.time()
        
        # Construct ffmpeg command with multi-threading
        cmd = [
            'ffmpeg', 
            '-threads', str(self.cpu_count),  # Use all CPU cores
            '-i', self.input_video,
            '-vf', f'fps={self.metrics["video_metrics"]["fps"]}',
            '-thread_queue_size', '512',  # Increase queue size for better throughput
    ...
    def compile_video(self):
        """Compile video with multi-threading support."""
        start_time = time.time()
        temp_video = self.output_dir / 'temp_output.mp4'
        input_name = Path(self.input_video).stem
        output_path = self.output_dir / f'{input_name}_full-v3_output.mp4'
        
        try:
            # Extract audio with threading
            audio_path = self.extract_audio()
            
            # Compile frames with multi-threading
            cmd = [
                'ffmpeg',
    ...
    def process_video(self):
        """Main processing pipeline with multi-threading support."""
        print("\n" + "="*80)
        print("VIDEO PROCESSING PIPELINE - MULTI-THREADED - DETAILED MONITORING")
        print("="*80)
        print(f"Using {self.cpu_count} CPU cores for processing")
        ...
        try:
            ...
            # Phase 3: Parallel Frame Processing
            print("\nPHASE 3: PARALLEL FRAME PROCESSING")
            print("="*50)
            ...
            if not first_frame_logo['detected']:
                raise Exception("Could not detect logo in first frame")
            ...
            with tqdm(total=len(frame_paths), desc="Processing frames") as pbar:
                for batch in frame_batches:
                    ...
                    logo_detected_count += sum(1 for r in results if r)
                    
                    pbar.update(len(batch))
                    pbar.set_postfix({
                        'CPU': f"{psutil.cpu_percent()}%",
                        'Memory': f"{psutil.Process().memory_info().rss/1024/1024:.1f}MB",
                        'Processed': f"{processed_count}/{len(frame_paths)}"
                    })
    ...
    def update_stats_file(self, message):
        ...
        stats_file = self.output_dir / 'full-v3_stats.txt'
